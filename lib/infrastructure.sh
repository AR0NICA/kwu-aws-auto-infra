...
launch_instances() {
    ...
    prepare_tomcat_identity || return 1
    tomcat_2a_user_data="$(build_tomcat_user_data "2A" "$DB_ENDPOINT" "$DB_SECRET_ARN")" || return 1
    tomcat_2c_user_data="$(build_tomcat_user_data "2C" "$DB_ENDPOINT" "$DB_SECRET_ARN")" || return 1
    ...
    aws_mutate "Attach Tomcat profile to 2A" ec2 associate-iam-instance-profile --instance-id "$TOMCAT_2A_INSTANCE_ID" --iam-instance-profile "Name=$TOMCAT_PROFILE_NAME" || return 1
    ...
    aws_mutate "Attach Tomcat profile to 2C" ec2 associate-iam-instance-profile --instance-id "$TOMCAT_2C_INSTANCE_ID" --iam-instance-profile "Name=$TOMCAT_PROFILE_NAME" || return 1
...
create_database() {
    ...
    aws_query DB_SECRET_ARN "Read RDS managed secret ARN" \
        rds describe-db-instances --db-instance-identifier "$DB_INSTANCE_IDENTIFIER" \
        --query 'DBInstances[0].MasterUserSecret.SecretArn' --output text || return 1
